 ...
 *     creación. Arrastrar una al área de clasificadas le asigna rango.
 *     Arrastrar una clasificada al área sin clasificar la desclasifica.
 *
 * El callback `onChangeOrden(classifiedIds)` recibe los ids de las clasificadas
 * en orden de rango 0..K-1 (bottom→top en términos visuales).
 */
export default function ListaDragDrop({
  ...
  // dropTarget: { section: "cla"|"sin", kind: "item"|"end", id?, before? }
  ...
  const computarNuevoOrden = useCallback((draggingId, target) => {
    ...
    // Si target.section === "sin", no se incluye en clasificadas
    // (queda fuera = se desclasifica).

    // Top de la lista visual = mayor rango. Invertimos para que bottom = rango 0.
  ...
  const handleDrop = useCallback((e, target) => {
    ...
    // Solo notificar si hubo un cambio efectivo
  ...
  function ItemFila({ id, idx, section, posicion }) {
    ...
    return (
      <div
        ...
        onDragOver={(e) => handleItemDragOver(e, section, id)}
        onDrop={(e) => handleDrop(e, { kind: "item", section, id, before: dropTarget?.before })}
        style={{
          position: "relative",
      ...
      >
        ...
        <span style={{
          flex: 1, fontSize: 13,
          color: t.nombre.trim() ? "var(--text-primary)" : "var(--text-muted)",
          fontStyle: t.nombre.trim() ? "normal" : "italic",
